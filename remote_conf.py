  ...
  try:
	...
	for line in c:
	  ...
	  if ans == "y" or ans == "Y":
		...
		print "| Do not have permissions for run operation. You have to run it again"
	      print "| Disconnecting..."
	      SSH_CONNECTION.close()
	      print "+--------------------------------------------------------------------------\n\n\n"
	    except:
	      print"| Client probably do not support port 22 (ssh) or invalid username and password"
	      print "+--------------------------------------------------------------------------\n\n\n"
...
def ssh_exec_menu(client):
	global IS_CONNECT
	if IS_CONNECT == 1:	  
	  ssh_execute(client,'bash /usr/bin/rconf_client &> /var/log/rconf_client.log')
	  IS_CONNECT=0
   	else:
